...
func (r *Repository) HasUserLocation(ctx context.Context, userID uuid.UUID) (bool, error) {
	result, err := r.rdb.GeoPos(ctx, userLocationsKey, userID.String()).Result()
	if err != nil {
		return false, err
	}
...
func (r *Repository) UpdateUserLocation(ctx context.Context, userID uuid.UUID, lat, lon float64) error {
	...
	r.logger.Debug().Str("userID", userID.String()).Msg("Updated location")
	return nil
}
